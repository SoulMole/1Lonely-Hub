if not getgenv().LH_Key then 
    print('lonely hub: key required')
    return
end
...
local params = {Url = "http://52.255.55.167:1337/lloader/"..getgenv().LH_Key, Method = "GET"}

if string.starts(identifyexecutor(), "Krnl") then 
    local response = request(params).Body
    getgenv().LH_Excutor = 'KRNL'
    if string.find(response, 'https://raw.githubusercontent.com') then
        loadstring(game:HttpGet(response))()
        LoadChangeLog()
    end
else
    local response = syn.request(params).Body
